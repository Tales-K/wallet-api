-- For transfers, ledger rows reference transfers.transfer_id; deposits/withdrawals have NULL transfer_id
CREATE TABLE IF NOT EXISTS ledger_entries (
  ...
  transfer_id      uuid NULL REFERENCES transfers(transfer_id) ON DELETE RESTRICT,
  ...
  CONSTRAINT chk_ledger_transfer_id_presence CHECK (
    (posting_type IN ('transfer_debit','transfer_credit') AND transfer_id IS NOT NULL)
    OR
    (posting_type IN ('deposit','withdraw') AND transfer_id IS NULL)
  )
);

-- ensure at most one debit and one credit per transfer_id
CREATE UNIQUE INDEX IF NOT EXISTS uq_transfer_once_per_side
  ON ledger_entries (transfer_id, posting_type)
